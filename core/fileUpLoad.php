<?php

/**
 * 上传图片核心类
 * @author <lensh>
 * @email  <986992484@qq.com>
 * @date   <2017.3.13>
 */
class FileUpLoad {

	private $result=array();

	/**
	 * 将批量上传之后的二维图片数组，改造成多个一维数组
	 * @return [type] [description]
	 */
	private function convertToOne(){
		$imgs=array();

		foreach ($_FILES['imgs']['name'] as $k => $v) {
			if($_FILES['imgs']['size'][$k]==0)  continue;
			$imgs[]=array(
				'name'=>$v,
				'type'=>$_FILES['imgs']['type'][$k],
				'tmp_name'=>$_FILES['imgs']['tmp_name'][$k],
				'error'=>$_FILES['imgs']['error'][$k],
				'size'=>$_FILES['imgs']['size'][$k]
			);
		}
		$_FILES=$imgs; //重新给超全局变量$_FILES赋值

		return $imgs;
	}

	/**
	 * 单张图片上传
	 * @param  [type] $imgName [description]
	 * @return [type]          [description]
	 */
	private function uploadOne($imgName){
		if(isset($_FILES[$imgName]) && $_FILES[$imgName]['error'] == 0){
			$name=$_FILES[$imgName]['name'];
			$tmp_name=$_FILES[$imgName]['tmp_name'];
			// extract($_FILES[$imgName]) 
			$path='../face/'.md5(uniqid()).strtolower(strstr($name, '.'));
			if(move_uploaded_file($tmp_name,$path)){
				$path=strstr($path,$path[3]);  //图片的存储地址
				return $path;	
			};
		}
	}

	/**
	 * 循环所有图片一张一张上传
	 * @return [type] [description]
	 */
	private function upload(){
		$imgs=$this->convertToOne(); //将批量上传之后的二维图片数组，改造成多个一维数组

		$this->result['paths']=array(); // 保存图片地址
		
		//循环所有的图片一张一张上传
		foreach ($imgs as $k => $v) {
			$this->result['paths'][]=$this->uploadOne($k);
		}
	}

	/**
	 * 上传到服务器后返回图片的地址
	 * @return [type] [description]
	 */
	public function getJSONPath($data){
		$this->upload();  //上传图片
		$this->result['user']=$data['user'];
		$this->result['pass']=$data['pass'];
		return json_encode($this->result);
	}
}

$f=new FileUpLoad();
echo $f->getJSONPath($_POST);
